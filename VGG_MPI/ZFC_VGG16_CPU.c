...
void read_weights(char *in_file, int lvls) {
    ...
    // Open the file only by rank 0 and broadcast data to others
    if (rank == 0) {
        iin = fopen(in_file, "r");
        if (iin == NULL) {
    ...
    // Broadcast the availability of the file to all ranks
    int file_available = (rank == 0 && iin != NULL) ? 1 : 0;
    MPI_Bcast(&file_available, 1, MPI_INT, 0, MPI_COMM_WORLD);
    if (file_available == 0) {
        MPI_Finalize();
        exit(1);
    }

    // Reading convolution weights (store them flipped from beginning)
    for (z = 0; z < 13; z++) {
        ...
        // Divide convolution weights loading among ranks
        for (i = rank; i < cshape[z][0]; i += size) {
            for (j = 0; j < cshape[z][1]; j++) {
                for (k = 0; k < cshape[z][2]; k++) {
                    for (l = 0; l < cshape[z][3]; l++) {
                        ...
                        MPI_Bcast(&dval, 1, MPI_FLOAT, 0, MPI_COMM_WORLD);
    ...
    for (z = 0; z < 3; z++) {
        ...
        // Divide dense weights loading among ranks
        for (i = rank; i < dshape[z][0]; i += size) {
        ...
        // Broadcast biases
        for (i = rank; i < dshape[z][1]; i += size) {
            if (rank == 0) {
                fscanf(iin, "%f", &dval);
            }
            MPI_Bcast(&dval, 1, MPI_FLOAT, 0, MPI_COMM_WORLD);
...
void dense(float *in, float **weights, float *out, int sh_in, int sh_out) {
    ...
    // Synchronize the output across all ranks
    MPI_Allreduce(MPI_IN_PLACE, out, sh_out, MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD);
}
...
void get_VGG16_predict(int only_convolution) {
    ...
    for (i = rank; i < cshape[level][0]; i += size) {
    ...
    // dump_memory_structure_dense_to_file(mem_block2_dense, dshape[level
